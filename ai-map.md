...
| --- | --- | --- | --- | --- |
| Ollama / local machine | `ai-chat`, `ai-embed`, `chat` | `granite4.1:3b`, `ministral-3:8b`, `embeddinggemma:latest` | Offline drafts, private text, quick experiments, embeddings, local preprocessing | Data stays on this machine. Quality and context are limited by local hardware/model. |
| OpenAI / API | `ai-chat`, `ai-transcribe`, `ai-embed` | `gpt-5.6`, `gpt-5.6-sol`, `whisper-1`, `text-embedding-3-small` | Repeatable scripted chat, transcription, embeddings, structured API workflows | Requires `OPENAI_API_KEY`; usage pricing and cloud data policies apply. |
...
| Anthropic / API | `ai-chat` | Catalog-listed Claude models such as `claude-sonnet-5` | Scripted chat and automation | Requires `ANTHROPIC_API_KEY`; usage pricing and cloud data policies apply. |
...
| Google Gemini / API | `ai-chat`, `ai-image`, `ai-speech`, `ai-image-analyze`, `ai-embed` | Catalog-listed Gemini models and capability-specific models | Scripted chat, image understanding/generation, speech, and embeddings | Requires `GOOGLE_API_KEY`; model capability and account availability vary. Cloud data policies and pricing apply. |
| CERIT-SC / e-INFRA CZ | `ai-chat`, `cerit-chat` | Maintained aliases such as `mini`, `coder`, `agentic`, `kimi`, `glm`, `deepseek` | Research, coding, large-context work, and approved institutional data | Requires `CERIT_API_KEY`. Exact model capabilities and service-side retention vary. |
